...
## Autorisations opérations
| URI | Méthode HTTP | Auths? | Opération |
|---|---|---|---|
| **`films`** | GET | Non | READ ALL : Lire toutes les ressources de la collection |
| **`films/:id`** | GET | Non | READ ALL FILTERED : Lire toutes les ressources de la collection |
| **`films/:id`** | DELETE | JWT | DELETE ONE : Supprimer une ressource basée sur les données de la requête |
| **`films/:id`** | PATCH | JWT | PATCH ONE : Modifier une partie d'une ressource basée sur les données de la requête |
| **`films/:id`** | PUT | JWT | PUT ONE : Modifier la totalité d'une ressource basée sur les données de la requête |
